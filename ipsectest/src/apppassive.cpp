...
int connectToServer(const std::string &ipAddress, int port)
{
    ...
    {
        std::cerr << "Connection Failed111" << std::endl;
        close(sockfd);
        return -1;
    }
...
void EncthreadFunction()
{
    ...
    int retries = 0;
    int conn_Proactive_APP_fd = -1;

    // 连接主动端APP
    while (conn_Proactive_APP_fd <= 0 && retries < max_retries)
    {
        conn_Proactive_APP_fd = connectToServer(proactiveAPP_ipAddress, APP_LISTEN_PORT);
        if (conn_Proactive_APP_fd <= 0)
        {
    ...
    label3: // 标签用于重新请求密钥
        ...
        // 发送到终端APP
        if (send(conn_Proactive_APP_fd, &ciphertext[0], ciphertext.length(), 0) == -1)
        {
            perror("send Error");
            close(conn_Proactive_APP_fd);
            return;
        }
        // 增加请求ID，防止重复使用相同的密钥
        request_id++;
